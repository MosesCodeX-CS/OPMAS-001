...
from config import ALARM_THRESHOLDS, DB_DRIVER, DB_HOST, DB_PORT, DB_USERNAME, DB_PASSWORD, DB_DATABASE, DB_PATH, LOG_FILE, PLC_HOST, PLC_PORT, POLL_INTERVAL, REGISTER_MAP, SCALE
...
def read_registers(client: ModbusTcpClient) -> dict[str, float | int | None]:
    ...
    result = client.read_holding_registers(address=start, count=count, device_id=1)
...
def check_alarms(connection, reading: dict[str, float | int | None]) -> None:
    thresholds = get_db_thresholds(connection)

    if reading['pressure'] is not None and reading['pressure'] < thresholds['pressure_low']:
        create_alarm(connection, 'Pressure', 'CRITICAL', f'Pressure below threshold: {reading["pressure"]} bar (Threshold: {thresholds["pressure_low"]} bar)')

    if reading['purity'] is not None and reading['purity'] < thresholds['purity_low']:
        create_alarm(connection, 'Purity', 'CRITICAL', f'Oxygen purity below threshold: {reading["purity"]}% (Threshold: {thresholds["purity_low"]}%)')

    if reading['temperature'] is not None and reading['temperature'] > thresholds['temperature_high']:
        create_alarm(connection, 'Temperature', 'WARNING', f'Temperature above limit: {reading["temperature"]} °C (Threshold: {thresholds["temperature_high"]} °C)')

    if reading['flow_rate'] is not None and reading['flow_rate'] < thresholds['flow_rate_low']:
        create_alarm(connection, 'Flow Rate', 'WARNING', f'Flow rate below threshold: {reading["flow_rate"]} (Threshold: {thresholds["flow_rate_low"]})')

    if reading['tank_level'] is not None and reading['tank_level'] < thresholds['tank_level_low']:
        create_alarm(connection, 'Tank Level', 'WARNING', f'Tank level below threshold: {reading["tank_level"]}% (Threshold: {thresholds["tank_level_low"]}%)')
...
def main() -> int:
    ...
    logger.info('Collector starting. PLC=%s:%s DB=%s', PLC_HOST, PLC_PORT, db_description)
